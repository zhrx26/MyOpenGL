...
int main()
{
	...
	// 创建封装过的Shader对象
	Shader pShader("2.3.materials.vs", "2.3.materials.fs");
	Shader pLampShader("2.3.lamp.vs", "2.3.lamp.fs");
	...
	while (!glfwWindowShouldClose(pWindow))
	{
		...
		// 青色塑料材质的箱子
		pShader.SetVec3("material.ambient", 0.0f, 0.1f, 0.06f);
		pShader.SetVec3("material.diffuse", 0.0f, 0.50980392f, 0.50980392f);
		pShader.SetVec3("material.specular", 0.50196078f, 0.50196078f, 0.50196078f);
		pShader.SetFloat("material.shininess", 32.0f);
		...
		//vLightColor.x = sin(glfwGetTime() * 2.0f);
		//vLightColor.y = sin(glfwGetTime() * 0.7f);
		//vLightColor.z = sin(glfwGetTime() * 1.3f);

		//glm::vec3 vDiffuseColor = vLightColor   * glm::vec3(0.5f); // 降低影响
		//glm::vec3 vAmbientColor = vDiffuseColor * glm::vec3(0.2f); // 很低的影响

		//pShader.SetVec3("light.ambient", vAmbientColor);
		//pShader.SetVec3("light.diffuse", vDiffuseColor);
		//pShader.SetVec3("light.specular", 1.0f, 1.0f, 1.0f);
		//pShader.SetVec3("light.position", vLightPos);
