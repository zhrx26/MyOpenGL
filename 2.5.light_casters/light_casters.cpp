...
int main()
{
	...
	// 创建封装过的Shader对象
	Shader pShader("2.5.light_casters.vs", "2.5.light_casters.fs");
	Shader pLampShader("2.5.lamp.vs", "2.5.lamp.fs");
	...
	while (!glfwWindowShouldClose(pWindow))
	{
		...
		// 平行光
		//pShader.SetVec4("light.lightVector", -0.2f, -1.0f, -0.3f, 1.0f);

		// 方向光
		//pShader.SetVec4("light.lightVector", glm::vec4(vLightPos, 0.0f));
		//// 光源覆盖50距离时，方向光的衰减系数
		//pShader.SetFloat("light.constant", 1.0f);
		//pShader.SetFloat("light.linear", 0.09f);
		//pShader.SetFloat("light.quadratic", 0.032f);

		// 聚光
		//pShader.SetVec4("light.lightVector", glm::vec4(vLightPos, 0.5f));
		//pShader.SetVec3("light.direction", glm::vec3(0.0f, 0.0f, -1.0f));
		//pShader.SetFloat("light.cutOff", glm::cos(glm::radians(12.5f)));

		//pShader.SetVec4("light.lightVector", glm::vec4(pCamera.Position, 0.5f));
		...
		pShader.SetVec3("light.direction", pCamera.Front);
		pShader.SetFloat("light.cutOff", glm::cos(glm::radians(12.5f)));
		pShader.SetFloat("light.outerCutOff", glm::cos(glm::radians(17.5f)));
		...
		glm::mat4 mLampModel;
		mLampModel = glm::mat4();
		mLampModel = glm::translate(mLampModel, vLightPos);
