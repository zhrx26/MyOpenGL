...
int main()
{
	...
	// 设置当模板测试失败、模板测试通过深度测试失败、模板测试和深度测试都通过，三种情况下的更新策略
	...
	Shader pShader("4.2.stencil_testing.vs", "4.2.stencil_testing.fs");
	Shader pSimpleColorShader("4.2.stencil_testing.vs", "4.2.simple_color.fs");
	...
	while (!glfwWindowShouldClose(pWindow))
	{
		...
		// DEPTH_BUFFER_BIT位来清除深度缓冲,GL_STENCIL_BUFFER_BIT位来清除模板缓冲
		...
		// 使用GL_ALWAYS模板测试函数，保证箱子的每个片段都会将模板缓冲的模板值更新为1
		glStencilFunc(GL_ALWAYS, 1, 0xFF);
		...
		glDisable(GL_DEPTH_TEST);  // 禁用了深度测试，防止边框不会被地板所覆盖
		pSimpleColorShader.Use();
		pSimpleColorShader.SetMat4("view", mView);
		pSimpleColorShader.SetMat4("projection", mProjection);
		float fScale = 1.1;
		mModel = glm::mat4(1.0f);
		mModel = glm::translate(mModel, glm::vec3(0.0f, 0.0f, -4.0f));
		mModel = glm::scale(mModel, glm::vec3(fScale, fScale, fScale));
		pSimpleColorShader.SetMat4("model", mModel);
		glDrawArrays(GL_TRIANGLES, 0, 36);
		glEnable(GL_DEPTH_TEST);
